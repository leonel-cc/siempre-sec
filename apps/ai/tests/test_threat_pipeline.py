...
class ThreatRuleTests(unittest.TestCase):
    ...
    def test_incident_rearms_only_after_continuous_absence(self):
        ...
        for _ in range(config.THREAT_CLEAR_OBSERVATIONS):
            self.engine.evaluate('cam', [person()])
        state = next(
            value for key, value in self.engine._states.items()
            if key.startswith('cam:weapon_detected:knife|')
        )
    ...
    def test_cached_weapon_observation_is_counted_once(self):
        detection = person(weapon='gun')
        detection['weapon']['_observation_id'] = 'weapon-frame-1'
        for _ in range(config.WEAPON_CONFIRMATIONS + 2):
            self.assertEqual(self.engine.evaluate('cam', [detection]), [])

    def test_weapon_confirmation_is_isolated_by_track(self):
        first = person(weapon='gun', tracking_id=1)
        second = person(weapon='gun', tracking_id=2)
        for _ in range(config.WEAPON_CONFIRMATIONS - 1):
            self.engine.evaluate('cam', [first])
        self.assertEqual(self.engine.evaluate('cam', [second]), [])
