...
class RuleEngine:
    ...
    def evaluate(self, camera_id: str, detections: List[Dict]) -> List[Dict]:
        ...
        for rule in self.rules:
            ...
            for key in keys:
                ...
                observation_id = (
                    observation['evidence'].get('_observation_id')
                    if observation else None
                )
                if (
                    observation_id is not None
                    and state['last_observation_id'] == observation_id
                ):
                    if state['active']:
                        self._mark_confirmed(
                            observation['detection'], rule_id,
                ...
                if rule_id == 'weapon_detected':
                    alert['weapon'] = evidence
                alerts.append(alert)
    ...
    def _collect_observations(self, detections: List[Dict]) -> Dict[str, Dict]:
        ...
        for detection in detections:
            ...
            if weapon:
                identity = weapon.get('associated_tracking_id')
                if identity is None:
                    identity = detection.get('tracking_id')
                if identity is None:
                    identity = 'standalone'
                subtype = f"{weapon['class']}|{identity}"
            ...
            if face_cover:
                ...
                if identity is None:
                    identity = 'standalone'
                subtype = f"covered|{identity}"
                current = observations['face_covered'].get(subtype)
                if current is None or face_cover['confidence'] > current['evidence']['confidence']:
                    observations['face_covered'][subtype] = {
                        'detection': detection,
                        'evidence': face_cover,
                    }
        return observations
